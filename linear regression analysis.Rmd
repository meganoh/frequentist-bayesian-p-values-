...
threegroups <- bind_cols(group_3_n_20, group_3_n_50, group_3_n_30, group_3_n_100)
```
...
##conf int 
```{r}
wald_2group <- wald_2group %>% mutate(ci = 
)
bayes_confint <- binom.bayes(wald_2group$sig, 1000, conf.level = 0.95) %>% select(-method, -n, )

confint <- binom.confint(wald_2group$sig, 1000, conf.level = 0.95, methods = "all") %>% pivot_wider(names_from = "method", values_from = c(mean:upper))
```
...
rm(wald_3group_n20, wald_3group_n30, wald_3group_n50, wald_3group_n100,
   ...
   likelihoodmax_3group_n20, likelihoodmax_3group_n30, likelihoodmax_3group_n50, likelihoodmax_3group_n100å)
```
