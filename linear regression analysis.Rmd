...
```{r}
#read in results 
tighter3_n20 <- bind_rows(results_3groups, .id = "id")
alpha <- 0.05 #set threshold for significance 
iter = 1000

threegroups <- bind_rows(threegroups, tighter3_n20, tighter3_n30, tighter3_n50, tighter3_n100)
```
...
save(threegroups, file = "simulation_2groups.rda")
```
...
twogroups <- twogroups %>% mutate(bf_prop = if_else(exp(bf) > 3, "null", "other"))
