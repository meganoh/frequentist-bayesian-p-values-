...
ggplot(ci2, aes(x = samplesize, y = mean)) + 
  geom_line() + geom_point(size = 0.5) + geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.5) + facet_grid(test ~ type)

ggplot(ci3, aes(x = samplesize, y = mean)) + 
  geom_line() + geom_pointrange(aes(ymin = lower, ymax = upper)) + facet_grid(test ~ type)
```
