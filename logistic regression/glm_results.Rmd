...
```{r}
group3n50 <- bind_rows(glmresults_3groups, .id = "id")
group3n1000$id <- as.numeric(group3n1000$id)
group3n1000 <- group3n1000 %>% filter(id <= 1000)
...
save(glmresults_3groups, file = "glm_3groups_n1000_iter1000.rda")
...
group3n50 <- group3n50 %>% mutate(bf_prop = if_else(exp(bf) > 3, "null", "no")) 

count <- group3n50 %>% 
  group_by(test) %>% count(bf_prop)
...
```{r}
ci2$samplesize_f <- as.factor(ci2$samplesize)
ggplot(ci2, aes(x = samplesize_f, y = ci$mean)) + 
  geom_line(aes(group = samplesize_f)) + 
  geom_point(size = .2)  + 
...
ggsave("glm_2group_CI.png", width = 30, height = 20, units = "cm")
...
ggsave("glm_3group_CI.png", width = 30, height = 20, units = "cm")
```
