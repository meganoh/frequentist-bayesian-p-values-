log_2groups <- function(i, sample_size, mod_iter, mod_warmup){
  ...
  out_tighterbayes <- data.frame(n = sample_size, 
                                 ...
                                 loo = loo_elpd_diff_tighter, 
                                 waic = waic_elpd_diff_tighter,
                                 bf = bf_tighter)
  out_widerbayes <- data.frame(n = sample_size, 
                                    test = "bayes_widerprior", pval = bayes_wider_pval,
                                    loo = loo_elpd_diff_wider, 
                                    waic = waic_elpd_diff_wider,
                                    bf = bf_wider) 
...
logrun_2groups <-  function(iter, sample_size, mod_iter, mod_warmup) {
  ...
  save(results_2groups, file = paste0("logistic_2groups", "_n", sample_size, "_iter", iter, ".rda"))
}
