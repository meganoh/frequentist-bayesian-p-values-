...
run <- function(iter, sample_size) {
  ...
  save(results, file = paste0("simulation", "_n", sample_size, "_iter", iter, ".rda"))
}
...
fit <- function(i, sample_size) {
  ...
  oosterwijk <- oosterwijk %>% 
    mutate(
      lp_oosterwijk_new = oosterwijk$lp__ -
        dlst(x = oosterwijk$b_Intercept, df = 3, 3, sigma = 0.102, mu = 0.350, log = TRUE) -
  ...
  p_oosterwijk <- pchisq(diff_student, df = 2, lower.tail = FALSE)
  p_oosterwijk_max <-pchisq(diff_student_max, df = 2, lower.tail = FALSE)
...
type.one.error<-replicate(n.simulations, t.test(rnorm(n.obs),rnorm(n.obs),
   var.equal=TRUE)$p.value)<alpha.p #number of times you reject the null hypothesis - TRUE, do not reject null hypothesis - FALSE

# type I error for the whole simulation - prob of rejecting null hypothesis 
ok <-data.frame(type.one.error) %>% count(type.one.error)
47/(953 + 47)
mean(type.one.error)
